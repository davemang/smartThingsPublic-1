 ...
 */
metadata {
	definition (name: "AeoTec Multisensor-kelty", namespace: "keltymd", author: "keltymd") {
...
def configureAfterSecure() {
    ...
    def request = [
		// send temperature, humidity, and illuminance every 1 minutes
		zwave.configurationV1.configurationSet(parameterNumber: 101, size: 5, scaledConfigurationValue: 128|64|32),
		zwave.configurationV1.configurationSet(parameterNumber: 111, size: 5, scaledConfigurationValue: 1*60),
    ...
    secureSequence(request) + ["delay 2000", zwave.wakeUpV1.wakeUpNoMoreInformation().format()]
}

def configure() {
	// log.debug "configure()"
    //["delay 3000"] + secure(zwave.securityV1.securityCommandsSupportedGet())
}
